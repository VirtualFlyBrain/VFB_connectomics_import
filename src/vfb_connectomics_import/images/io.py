...
#:
#: `volume.dps.pkl` is a navis Dotprops pickle, the input to NBLAST, generated by
#: VFB_similarity_import's cache_runner and never written here. It MUST be deleted.
#: That job decides whether to reprocess a neuron by comparing mtimes — `is_neuron_cached`
#: in score_grabber.py keeps the dps if `mtime(dps) >= mtime(swc)` — so a freshly swapped
#: SWC would normally be caught on its own. Deleting the dps anyway removes the dependency
#: on mtime, which is the fragile part: a copy that preserves timestamps, NAS clock skew,
#: or anything writing an SWC with a backdated mtime all make a stale dps compare as fresh,
#: silently and with no error. An absent file cannot. This matters more than a stale
...
#:
#: Consequence worth knowing: after a full run these neurons have NO dps until cache_runner
#: is re-run for each template, and it only rebuilds neurons its KB query returns.
#:
